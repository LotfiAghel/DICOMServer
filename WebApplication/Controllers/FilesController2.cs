...
namespace WebApplication.Controllers
{
    ...
    public class Files2Controller : UsersBaseController
    {
        ...
        [Route("api/[controller]/DownloadReviseFile/{reviseId}/{*Name}")]
        [HttpGet]
        [Authorize]
        public async Task<ActionResult> DownloadReviseFile(Guid reviseId,string Name)
        {
            var any=await dbContext.responseAdjusts.Where(x => x.id == reviseId &&
                                                 (x.examinerId == getUserId() || x.response.examPartSession
                                                                                  .CustomerId == getUserId()
                                                                              || x.examinerId == getUser2Id() ||
                                                                              x.response.examPartSession.CustomerId ==
                                                                              getUser2Id()
                                                 )
            ).FirstOrDefaultAsync();
            ...
            var path = Path.Combine(JsonBase64File.UserUploadFolderPath, "revise",any.examinerId.ToString(), any.id.ToString(),Name);
            ...
            var file = new JsonBase64File(path,Name);
            var fileExtension = Path.GetExtension(path).TrimStart('.').ToLower();
