...
namespace WebApplication.Controllers
{
    ...
    public class UsersBaseController : Controller, IUserServiceBase
    {
        ...
        public LearnBranch getLearnBranch()
        {

            if (HttpContext.Request.Headers.TryGetValue("learn_branch", out var values))
            {

                if (int.TryParse(values.ToString(), out var numericValue))
                {
                    if (Enum.IsDefined(typeof(LearnBranch), numericValue))
                    {
                        return (LearnBranch)numericValue;
                    }
                }
                else if (Enum.TryParse<LearnBranch>(values.ToString(), true, out var enumValue))
                {
                    return enumValue;
                }
            }
            if (HttpContext.Request.Headers.TryGetValue("LearnBranch", out var values2))
            {

                if (int.TryParse(values2.ToString(), out var numericValue))
                {
                    if (Enum.IsDefined(typeof(LearnBranch), numericValue))
                    {
                        return (LearnBranch)numericValue;
                ...
                else if (Enum.TryParse<LearnBranch>(values2.ToString(), true, out var enumValue))
                {
                    return enumValue;
                }
            }

            return LearnBranch.Toefl;
        }
    ...
    public class UsersController : UsersBaseController
    {
        ...
        static tools.KaveNegarSms ks = new tools.KaveNegarSms();
        ...
        public virtual async Task<ActionResult<OTPLoginRes>> LoginByMobileVerifyCode([FromBody] LoginWithOTP model)
        {
          ...
          && x.phoneNumber == model.PhoneNumber && x.code == int.Parse(model.OTP)).FirstOrDefaultAsync();
            ...
            }
            {
                ...
                if (_oldContex != null)
                {
                    var oldUSer = await _oldContex.Customers.Where(x => x.UserName == model.PhoneNumber).FirstOrDefaultAsync();
                    if (oldUSer == null)
                    {
                        AccountService _accountService = new AccountService(_oldContex, null, null, null);
                        var res = await _accountService.Register(model.PhoneNumber, "beta", EnglishToefl.Models.DeviceType.Web);
                        if (res.Success)
                        {
                            oldUSer = await _oldContex.Customers.Where(x => x.UserName == model.PhoneNumber).FirstOrDefaultAsync();
                        }
                    }

                    if (oldUSer != null)
                    {
                        user = await UserMigrator.MigrateUser(_oldContex, _context, oldUSer);
                        //UserMigrator.migrateUserExamAndLeitnerStart(user.id);
                    }
                }
        ...
        {
            if(model.pass.IsNullOrEmpty() || model.pass.Length<8)
        ...
        [HttpGet("getExamId")]
        public async Task<int> getExamId()
        {
            var t= await _context.physicalExamClientRels.Where(x=>x.userId==this.getUserId() && x.PhysicalExam.isActive).OrderByDescending(x=> x.createdAt).Select(x=>x.examId).FirstOrDefaultAsync();
            if(t==0)
                t= await _context.physicalExamClientRels.Where(x=>x.userId==this.getUserId() ).OrderByDescending(x=> x.createdAt).Select(x=>x.examId).FirstOrDefaultAsync();
            return t; 
        }
        ...
        public class LoginTimeOutExeption : Exception
        {
        ...
        public async Task<ClientMsgs.LoginResponse> oldLogin0([FromBody] JwtReQ jwt)
        {
            ...
            {
                if(_oldContex!=null){
                    var customer = await _oldContex.Customers.FindAsync(userId);


                    user = await UserMigrator.MigrateUser(_oldContex, _context, customer);
                }
            }
            catch (Exception e)
            {
        ...
        public async Task<ActionResult<ClientMsgs.GetUserResponse>> getUser0()
        {
            ...
            var br=this.getLearnBranch();
            //await ExamMigrator.migrateServiceBuys(_oldContex,_context,this.getUserId());
            var q = _context.ServiceBuys.Where(x => x.CustomerId == nUser.id).Include(x => x.Service);
            DateTime neoTime = DateTime.MinValue;
            DateTime IeltsTime = DateTime.MinValue;
            DateTime GreTime = DateTime.MinValue;
            
            DateTime leitnerTime = DateTime.MinValue;
            try
            {
                neoTime = await q.Where(x => x.Service.NumberOfDolphin >= 10).Where(x=> x.Service.ServiceGroup.branches.Contains(LearnBranch.Toefl)).MaxAsync(x => x.ValidUntil);
            }
            catch { }
            try
            {
                IeltsTime = await q.Where(x => x.Service.NumberOfDolphin >= 10).Where(x=> x.Service.ServiceGroup.branches.Contains(LearnBranch.Ielts)).MaxAsync(x => x.ValidUntil);
            }
            catch { }
            try
            {
                GreTime = await q.Where(x => x.Service.NumberOfDolphin >= 10).Where(x=> x.Service.ServiceGroup.branches.Contains(LearnBranch.GRE)).MaxAsync(x => x.ValidUntil);
            }
            catch { }
            try
            {
                leitnerTime = await q.Where(x => x.Service.NumberOfLeitnerAdd >= 60).MaxAsync(x => x.ValidUntil);
            }
            catch { }
            ...
            if (br == LearnBranch.GRE)
                rr.branchTime = GreTime;
            if (br == LearnBranch.Ielts)
                rr.branchTime = IeltsTime;
            if (br == LearnBranch.Toefl)
                rr.branchTime = neoTime;
        ...
        static IQueryable<ExamPartSession> filter(IQueryable<ExamPartSession> q)
        {
            return q.Where(x =>
                (x.SectionType == EnglishToefl.Models.ExamPartType.Speaking && x.responses.Any())
                || (x.SectionType == EnglishToefl.Models.ExamPartType.Writing && x.responses.Any())
                || (x.responses.GroupBy(x => x.QuestionId).Count() > 0)
            )
                .Where(x => x.dataSource != DataSource.MigratedFromOldServer && x.dataSource != DataSource.CreateInNewServerForOldserverMigration);
        }

        [HttpGet("examPartSessions/{examId}")]
        [IgnoreDocAttribute]
        public async Task<List<Models.ExamPartSession>> examPartSessions(int examId, [FromQuery] DateTime? date = null)
        {
            //if (!await checkPermission<ViewAccess>()) return Unauthorized(); //TODO check select access detail Entity
            //var thisEntity = await getNUser();
            {
                //var customer = await _context.users.Where(x => x.id == getUserId()).FirstOrDefaultAsync();
                //await UserMigrator.migrateUserExam(_oldContex, _context, customer, examId);
            }
            //var thisEntity = await _context.users.FindAsync(getUserId());
            var q = _context.ExamPartSessions.AsNoTracking().Where(x => x.CustomerId == getUser2Id());

            if (date != null)
                q = q.Where(x => x.startTime >= date);
            q = filter(q);
            return await q.AsNoTracking().Where(x => x.examId == examId)
                .OrderByDescending(x => x.startTime).ToListAsync();

        }
        [HttpGet("examSessions/{examId}")]
        [IgnoreDocAttribute]
        public async Task<List<Models.ExamSession>> examSessions(int examId)
        {
            //if (!await checkPermission<ViewAccess>()) return Unauthorized(); //TODO check select access detail Entity
            var thisEntity = await _context.users.FindAsync(getUserId());
            var q = _context.Entry(thisEntity).Collection(x => x.examSessions).Query();
           
            return await q.Where(x => x.examId == examId).OrderByDescending(x => x.startTime).Take(10).ToListAsync();

        }

        [HttpGet("examPartSessions2/{examId}/{sectionType}")]
        [IgnoreDocAttribute]
        public async Task<List<Models.ExamPartSession>> examPartSessions(int examId, EnglishToefl.Models.ExamPartType sectionType)
        {
            //if (!await checkPermission<ViewAccess>()) return Unauthorized(); //TODO check select access detail Entity
            var thisEntity = await _context.users.FindAsync(getUser2Id());
            var q = _context.Entry(thisEntity).Collection(x => x.examPartSessions).Query();
            
            q = filter(q);
            return await q.Where(x => x.examId == examId && x.SectionType == sectionType).OrderByDescending(x => x.startTime).ToListAsync();
        }


        List<EnglishToefl.Models.Exam> exams = null;
       
        [HttpGet("haveExamPartSessions")]
        [IgnoreDocAttribute]
        public async Task<List<ExamPart>> haveExamPartSessions()
        {
            //if (!await checkPermission<ViewAccess>()) return Unauthorized(); //TODO check select access detail Entity
            var thisEntity = await _context.users.FindAsync(getUser2Id());
            ...
            try
            {
                ...
                var x=UserMigrator.migrateUserExamAndLeitnerStart(this.getUser2Id());
                var udr = new UserRamCacheDataRepository(_context);
                var ud = await udr.getOrCreateData(this.getUser2Id());
                if (ud.lastResponse > ud.lastResponseCalculation)
                {
                    await udr.createExamProgressBar(_context, this.getUser2Id());
                }
                await udr.calcSubjectData(_context, this.getUser2Id());

            }
            catch
            {
                Console.WriteLine(" MIGRATIONS NOT WORK "); //TODO
            }
            

            var q = _context.ExamPartSessions.Where(x => x.CustomerId == thisEntity.id);



            q = filter(q);
            var q2 = q.GroupBy(x => new { x.examId, x.SectionType }).Where(x => x.Any())
                .Select(x => x.Key).Select(x => new ExamPart(new ForeignKey2<Models.Exam, int>(x.examId), x.SectionType,0));
            //var q2 = _context.ExamPartSessions.Where(x => x.CustomerId == thisEntity.id).GroupBy(x => x.examId).Where(x => x.Any()).Select(x => x.First().examId);
            var q3 = await q2.ToListAsync();
            try
            {
                if (exams == null)
                    exams = await _oldContex.Exams.ToListAsync();
                var responses = await _oldContex.Responses.AsNoTracking().Where(x => x.CustomerId == thisEntity.id.ToString() && !x.IsRemoved)
                  .Select(x => new { x.ExamName, x.SectionType }).Distinct().ToListAsync();

                responses.ForEach(x =>
                {
                    var exam = exams.FirstOrDefault(y => y.Name == x.ExamName);
                    if (exam != null)
                        q3.Add(new ExamPart(new ForeignKey2<Models.Exam, int>(exam.id), x.SectionType));
                });
            }
            catch
            {
        ...
        public async Task<string> progress()
        {
            ...
            var udr = new UserRamCacheDataRepository(_context);
            var t=await udr.createExamProgressBar(_context, this.getUser2Id());
            await udr.calcSubjectData(_context, this.getUser2Id());
        ...
        [HttpGet("examDatas")]
        [IgnoreDocAttribute]
        public async Task<List<ExamPart>> examDatas()
        {
            //if (!await checkPermission<ViewAccess>()) return Unauthorized(); //TODO check select access detail Entity
            var thisEntity = await _context.users.FindAsync(getUserId());
            ...
            var q = _context.ExamPartSessions.Where(x => x.CustomerId == thisEntity.id).Include(x => x.responses).Select(x => new { x,count=x.responses.Count()});



            //q = filter(q);

            var q2 = q.GroupBy(x => new { x.x.examId, x.x.SectionType });//.Where(x => x.Any());

            var q3=q2
                .Select(x => x.OrderByDescending(x => x.x.startTime).First())
            
                //.Select(x => x.examId)
                .ToList();
            return  q3
                //.Select(x => x.OrderByDescending(x => x.startTime).First())
                .Select(x => new ExamPart(new ForeignKey2<Models.Exam, int>(x.x.examId), x.x.SectionType,  x.count ))
                .ToList();
        ...
        [HttpGet("SessionsDetail/{sessionId}")]
        [IgnoreDocAttribute]
        public async Task<List<Models.Response>> SessionsDetail(Guid sessionId)
        {
            ...
            var t = await _context.ExamPartSessions.FindAsync(sessionId);
            if (t.CustomerId != this.getUserId())
                return null;

            var ll = await _context.Responses.Where(x => x.examPartSessionId == sessionId).Include(x => x.question)
                .GroupBy(x => x.QuestionId).Select(x => x.OrderBy(x => x.EnterDate).Last())
                .ToListAsync();
            foreach (var r in ll)
                r.clacl();
            return ll;
        ...
        [HttpGet("SessionsDetail2/{sessionId}")]
        [IgnoreDocAttribute]
        public async Task<List<Models.Response>> SessionsDetail2(Guid sessionId)
        {
            ...
            var t = await _context.ExamPartSessions.FindAsync(sessionId);
            if (thisEntity.userType!=UserType.MockThird && t.CustomerId != this.getUser2Id() )
                return null;
            var access=await _context.userExaminers.Where(x => x.userId == t.CustomerId && x.examinerId == this.getUser2Id()).AnyAsync();
            if(!access && t.CustomerId != this.getUser2Id())
                return null;
        ...
        [HttpGet("SessionsResponses/{sessionId}")]
        public async Task<List<Models.Response>> SessionsResponses(Guid sessionId)
        {
            //if (!await checkPermission<ViewAccess>()) return Unauthorized(); //TODO check select access detail Entity
            var thisEntity = await _context.users.FindAsync(getUser2Id());
            var q = _context.Entry(thisEntity).Collection(x => x.examPartSessions).Query();
            ...
            var t = await _context.ExamPartSessions.FindAsync(sessionId);
            if (t.CustomerId != this.getUser2Id())
                return null;
            var ll = await _context.Responses.Where(x => x.examPartSessionId == sessionId).Include(x => x.question)
                .ToListAsync();
            return ll;
        ...
        {
            if(model.name.IsNullOrEmpty() || model.name.Length>20)
        ...
        public virtual async Task<ActionResult<List<IIdMapper<int>>>> searchContent([FromBody] SearchContent model)
        {

            var sq = _context.SectionParts.Where(x => x.Section.exam.company.learnBranch == getLearnBranch());
            var qq = _context.Questions.Where(x => x.section.exam.company.learnBranch == getLearnBranch());
            List<IIdMapper<int>> z = [];
            
            z.AddRange(sq.Where(x => x.name.ToLower().Contains(model.text.ToLower().Trim())).OrderBy(x=> x.id).Take(20).ToList());
            z.AddRange(sq.Where(x => x.Content.ToLower().Contains(model.text.ToLower().Trim())).OrderBy(x=> x.id).Take(20).ToList());
            
            z.AddRange(qq.Where(x => x.QuestionText.ToLower().Contains(model.text.ToLower().Trim())).OrderBy(x=> x.id).Take(20).ToList());
        ...
        [HttpPost("searchSectionPart")]
        public virtual async Task<ObjectContainerRespone<List<SectionPart>>> searchSectionPart([FromBody] SearchContent model)
        {

            var txt=model.text.ToLower().Trim();
            if (txt.Length < 5)
                return new ObjectContainerRespone<List<SectionPart>>() { done = false, text = "text is too short" };
            int[] h = [1,7, 11,13, 14];
            
            var sq = _context.SectionParts.Where(x => x.Section.exam.company.learnBranch == getLearnBranch() && !h.Contains(x.Section.exam.CompanyId));
            
            if (model.examPartTypes != null)
                sq = sq.Where(x => model.examPartTypes.Contains(x.Section.ExamPartType));
            
            if(model.examCompanies!=null)
                sq = sq.Where(x => model.examCompanies.Contains(x.Section.exam.CompanyId));
            
            var res= await sq.Where(x =>
                x.Content.ToLower().Contains(model.text.ToLower())
                || (x.subtitile != null && x.subtitile.Any(x => x.text.ToLower().Contains(model.text.ToLower())))
            ).Take(100).ToListAsync();
            return new ObjectContainerRespone<List<SectionPart>>() { done=true, data = res };
        ...
        public class ZA
        {
            public List<SectionPart> sp { get; set; }
            public List<Question> qs { get; set; }
        }
        
        [HttpPost("searchSectionPart2")]
        public virtual async Task<ObjectContainerRespone<ZA>> searchSectionPart2([FromBody] SearchContent model)
        {

            var txt=model.text.ToLower().Trim();
            if (txt.Length < 5)
                return new ObjectContainerRespone<ZA>() { done = false, text = "text is too short" };
            int[] h = [1,6,7, 11,13, 14];
            
            var sq = _context.SectionParts.Where(x => x.Section.exam.company.learnBranch == getLearnBranch() && !h.Contains(x.Section.exam.CompanyId));
            var qs = _context.Questions.Where(x => x.section.exam.company.learnBranch == getLearnBranch() && !h.Contains(x.section.exam.CompanyId));

            if (model.examPartTypes != null)
            {
                sq = sq.Where(x => model.examPartTypes.Contains(x.Section.ExamPartType));
                qs = qs.Where(x => model.examPartTypes.Contains(x.section.ExamPartType));
            }

            if (model.examCompanies != null)
            {
                sq = sq.Where(x => model.examCompanies.Contains(x.Section.exam.CompanyId));
                qs = qs.Where(x => model.examCompanies.Contains(x.section.exam.CompanyId));
            }
            ...
            var res2= await qs.Where(x =>
                x.Content.ToLower().Contains(model.text.ToLower())
            ).Take(100).ToListAsync();
            
            return new ObjectContainerRespone<ZA>() { done=true, data = new ZA()
            {
                sp = res,
                qs=res2
            } };
