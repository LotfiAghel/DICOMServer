...
namespace Models
{
    ...
    public class SearchByFileId : IQuery<EnglishToefl.Models.Response>
    {
        public string multiMedia { get; set; }
        public IQueryable<EnglishToefl.Models.Response> run(IQueryable<EnglishToefl.Models.Response> q)
        {
            return q.Where(x => x.Multimedia == multiMedia);
        }
    }
    ...
    public class SmsListQuery : ListQuery<EnglishToefl.Models.SendSmsHistory> { };
    ...
    public class ServiceBuySearchAg: IQueryConvertor<EnglishToefl.Models.ServiceBuy,CC>
    {
        public IQueryable<CC> run(IQueryable<EnglishToefl.Models.ServiceBuy> q)
        {
            return q.GroupBy( x => new DateTime (x.BuyTime.Year, x.BuyTime.Month, x.BuyTime.Day )).Select(x =>
            new CC
            {
                day=x.Key,
                PaidPrice = x.Sum(y=>y.PaidPrice)
            });
        }
    }
